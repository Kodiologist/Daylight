...
Here's [[m:i][italics]] implemented with ~m:i~. Here's [[m:code][computer-code markup]] implemented with ~m:code~. Here we try using an unbracketed link inside a link of that kind: [[m:i][http://google.com]]. (Unfortunately, Org can't handle nested double brackets.)
